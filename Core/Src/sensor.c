...
static HAL_StatusTypeDef LSM6DSR_ReadRegistersDMA(uint8_t reg, uint8_t *data, uint16_t length)
{
  uint32_t spi2_timeout_start;

  if ((length == 0U) || ((uint32_t)length >= sizeof(spi2_who_am_i_tx_buffer)))
  {
    return HAL_ERROR;
  }

  spi2_who_am_i_ready = 0U;
  spi2_who_am_i_error = 0U;
  spi2_who_am_i_tx_buffer[0] = reg | 0x80U;
  memset(&spi2_who_am_i_tx_buffer[1], 0, length);
  memset(spi2_who_am_i_rx_buffer, 0, length + 1U);
  ...
  if (HAL_SPI_TransmitReceive_DMA(&hspi2, spi2_who_am_i_tx_buffer, spi2_who_am_i_rx_buffer, length + 1U) != HAL_OK)
  {
    IMU_CS_HIGH();
    return HAL_ERROR;
  }

  spi2_timeout_start = HAL_GetTick();

  while ((spi2_who_am_i_ready == 0U) && (spi2_who_am_i_error == 0U))
  {
    if ((HAL_GetTick() - spi2_timeout_start) > 100U)
    {
      HAL_SPI_Abort(&hspi2);
      IMU_CS_HIGH();
  ...
  if (spi2_who_am_i_ready != 1U)
  {
    return HAL_ERROR;
  }

  memcpy(data, &spi2_who_am_i_rx_buffer[1], length);

  return HAL_OK;
}

static HAL_StatusTypeDef LSM6DSR_ReadMotion(lsm6dsr_raw_data_t *motion)
{
  uint8_t motion_buffer[LSM6DSR_MOTION_BURST_LENGTH];

  if (LSM6DSR_ReadRegistersDMA(LSM6DSR_OUTX_L_G_REG, motion_buffer, sizeof(motion_buffer)) != HAL_OK)
  {
    return HAL_ERROR;
  }
